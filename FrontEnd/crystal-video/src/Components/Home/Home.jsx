...
export default function Home() {
  ...
  return (
    <>
      {/* Intro text */}
      <div className={`${styles.introContainer} ${styles.backgroungImg} ${styles.backgroungstyle}` }>
        <div className={styles.introText}>
          {messages.slice(0, messageIndex + 1).map((msg, index) => (
           <p key={index} className={`${styles.fancy} ${styles.homeText} ${styles.HomeP}  animate__animated animate__bounceInLeft`}>{msg}</p>
          ))}
        </div>
      </div>
